...
async function listiraamatud() {
    const responseData = await getDataAsJson("https://localhost:5000/raamatud/");
    const resultElement = document.getElementById("raamatud_result");
    resultElement.innerHTML = ""
    for (var raamat of responseData.raamatud){
        resultElement.innerHTML += '<a href="https://localhost:5000/raamatud/'+raamat+'"  download="'+raamat+'.txt" >' +raamat+".txt</a> " +
                '<a href="#" onclick="deleteObject(\'https://localhost:5000/raamatud/'+raamat+'\')" > [kustuta]</a>' +
                "<br />";
    }
}
